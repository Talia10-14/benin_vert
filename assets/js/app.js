...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        // Note: sw.js est à la racine, donc le chemin reste /sw.js ou ./sw.js par rapport à la racine
...
const appData = data; // Imported data
...
// --- INITIALIZATION ---
function init() {
    // Bind navigation events (remplacement des onclick="router(...)" inline)
    document.querySelectorAll('.nav-item').forEach(item => {
...
function renderHome() {
  ...
  `;
    ...
    categories.forEach(cat => {
        ...
        card.onclick = () => router(cat.action);
        card.innerHTML = `
...
function renderPlants() {
    ...
    // Header
    // Note: Using textContent where possible would be safer, but here we construct HTML structure
    container.innerHTML = `
    ...
    setTimeout(() => {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', (e) => {
                ...
                currentState.searchQuery = e.target.value; // Update state globally for this view re-renders?
                // Simple client-side filtering without re-render for performance here
